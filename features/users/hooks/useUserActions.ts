...
import { UserSchema, type UpdateUserInput, type User, type Role } from "@/features/users/schemas/user.schema";
...
/** Replaces the full set of roles assigned to a user (management-only, per backend rules). */
export function useUpdateUserRoles() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, roles }: { id: string; roles: Role[] }): Promise<User> => {
      const { data } = await clientApi.put(ENDPOINTS.users.roles(id), { roles });
      return UserSchema.parse(data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: userKeys.lists() });
