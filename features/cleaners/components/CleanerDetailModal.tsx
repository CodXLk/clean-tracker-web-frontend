...
import { Mail, Phone, Building2, MapPin, Loader2 } from "lucide-react";
import { Modal } from "@/components/shared/Modal";
import { UserAvatar } from "@/components/shared/UserAvatar";
import { UserDocumentsSection } from "@/features/users/components/UserDocumentsSection";
import { useCleanerDetailByUser } from "@/features/cleaners/hooks/useCleanerDetail";
import { SITE_TYPE_LABELS } from "@/features/user-management/schemas/site.schema";
...
function fullName(first?: string | null, last?: string | null): string {
  return [first, last].filter(Boolean).join(" ").trim() || "Unnamed cleaner";
}

export function CleanerDetailModal({ open, onClose, user }: CleanerDetailModalProps) {
  const detailQuery = useCleanerDetailByUser(open ? user?.id : undefined);
  ...
  return (
    <Modal
      ...
      description="Contact details, compliance documents and assigned sites."
    ...
    >
      {detailQuery.isLoading ? (
        ...
        <p className="rounded-lg bg-error/10 px-3 py-2 text-sm font-medium text-error">
          Failed to load cleaner profile.
        </p>
      ) : (
        <div className="flex flex-col gap-6">
                ...
                {detail.sites.map((site) => (
                  <li key={site.siteId} className="rounded-xl border border-grey-200 p-3">
                    ...
                    <div className="mt-1 flex flex-col gap-0.5 text-xs text-grey-500">
                      ...
                      <span>
                        {site.siteType ? SITE_TYPE_LABELS[site.siteType] : "Site"}
                        {site.slotLabel ? ` · ${site.slotLabel}` : ""}
                      </span>
                    </div>
                  </li>
                ))}
          ...
          {detail.userId && (
            <div className="border-t border-grey-100 pt-4">
              <UserDocumentsSection userId={detail.userId} />
            </div>
          )}
        </div>
      )}
