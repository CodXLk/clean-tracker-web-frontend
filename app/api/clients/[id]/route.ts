...
export async function GET(_request: NextRequest, ctx: RouteContext<"/api/clients/[id]">) {
  ...
  return proxyBackend(BACKEND.clients.byId(id), { method: "GET" });
}

export async function PUT(request: NextRequest, ctx: RouteContext<"/api/clients/[id]">) {
  const { id } = await ctx.params;
  const body = await request.json();
  return proxyBackend(BACKEND.clients.byId(id), { method: "PUT", body });
}

export async function DELETE(_request: NextRequest, ctx: RouteContext<"/api/clients/[id]">) {
  const { id } = await ctx.params;
  return proxyBackend(BACKEND.clients.byId(id), { method: "DELETE" });
}
