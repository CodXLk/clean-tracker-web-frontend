...
export async function GET(_request: NextRequest, ctx: RouteContext<"/api/users/[id]">) {
  ...
  return proxyBackend(BACKEND.users.byId(id), { method: "GET" });
}

export async function PUT(request: NextRequest, ctx: RouteContext<"/api/users/[id]">) {
  const { id } = await ctx.params;
  const body = await request.json();
  return proxyBackend(BACKEND.users.byId(id), { method: "PUT", body });
}

export async function DELETE(_request: NextRequest, ctx: RouteContext<"/api/users/[id]">) {
  const { id } = await ctx.params;
  return proxyBackend(BACKEND.users.byId(id), { method: "DELETE" });
}
