...
// Multipart needs the original FormData + boundary, so proxyBackend can't be used here.
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.SPRING_BOOT_API_URL}${BACKEND.tasks.draftPhotos}`, {
      method: "POST",
